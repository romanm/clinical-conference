...
public class JpaBookingService implements BookingService, Serializable {
    ...
    @Transactional(readOnly = true)
    @SuppressWarnings("unchecked")
    public List<Hotel> findHotels(SearchCriteria criteria, int firstResult, String sortBy, SortOrder orderDirection) {
	String pattern = getSearchPattern(criteria);
	String orderBy = " ORDER BY h.";
	...
	return em
		.createQuery(
			"select h from Hotel h where lower(h.name) like :pattern or lower(h.city) like :pattern "
				+ "or lower(h.zip) like :pattern or lower(h.address) like :pattern " + orderBy
				+ orderDirection).setParameter("pattern", pattern)
		.setMaxResults(criteria.getPageSize()).setFirstResult(firstResult).getResultList();
    }
